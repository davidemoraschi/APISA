/* Formatted on 22/01/2014 13:34:01 (QP5 v5.163.1008.3004) PANDAS Copyright (c) 2013 -Davide Moraschi (davidem@eurostrategy.net)
	...
	SELECT CC.ORGANICA NATID_ORGANOGESTOR
		...
		FROM REP_PRO_SIGLO.ORG_CENTROCONSUMO@SYG CC
				 ...
				 LEFT JOIN REP_PRO_SIGLO.ORG_UNIDADGESTIONCLINICA@SYG UGC1
					 ON (UGCSV.UNIDADGESTIONCLINICA = UGC1.ID)
...
CONSTRAINT MSTR_DET_REPARTO_CC_SV_UGC_R03
 ...
 VALIDATE
/
SELECT COUNT (DISTINCT NATID_CENTROCONSUMO) FROM MSTR_DET_REPARTO_CC_SV_UGC;

BEGIN
	SYS.DBMS_STATS.GATHER_TABLE_STATS (OwnName => 'MSTR_SIGLO'
																		,TabName => 'MSTR_DET_REPARTO_CC_SV_UGC'
																		,Estimate_Percent => 0
