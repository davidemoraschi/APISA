...
CREATE OR REPLACE PROCEDURE send_to_SP2013_long_text (
...
BEGIN
   ...
   p_url :=
      ...
      || '&reportID='
      || reportId;
   DBMS_LOB.createtemporary (l_report_clob, FALSE);
   ...
   p_url :=
      'http://sp13pruebas.sas.junta-andalucia.es/_api/web/GetFolderByServerRelativeUrl(''/Documentos%20compartidos'')/Files/add(url=''50k.txt'',overwrite=true)'; /*OK*/
   ...
   body_length := DBMS_LOB.GETLENGTH (l_report_clob);

   IF body_length <= 32767
   THEN
      UTL_HTTP.set_header (l_http_request, 'Content-Length', body_length);
      --UTL_HTTP.WRITE_RAW (l_http_request, l_report_clob);
      UTL_HTTP.WRITE_TEXT (l_http_request, l_report_clob);
   ...
   DBMS_LOB.freetemporary (l_report_clob);
