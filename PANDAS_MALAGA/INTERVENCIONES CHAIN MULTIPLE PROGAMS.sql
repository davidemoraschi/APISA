   ...
   LOOP
      ...
      BEGIN
         DBMS_SCHEDULER.CREATE_PROGRAM (program_name     => v_program_name,
                                        ...
                                        enabled          => TRUE,
                                        comments         => 'Carga de Intervenciones del área ' || C1.NATID_AREA_HOSPITALARIA);
      END;
   END LOOP;
END;
/
...
BEGIN
   ...
   DBMS_SCHEDULER.CREATE_PROGRAM (program_name     => 'INTERV_ALL_STEP02',
                                  ...
                                  --number_of_arguments   => 1,
                                  enabled          => TRUE,
                                  comments         => 'MERGE de Intervenciones de todas las áreas');
END;
/
...
BEGIN
   DBMS_SCHEDULER.CREATE_CHAIN (chain_name            => 'PANDAS_INTERV',
                                ...
                                comments              => 'Cadena de ejecuciones para Intervenciones a partir de las réplicas');
END;
/
...
BEGIN
   DBMS_SCHEDULER.CREATE_JOB (job_name          => 'ETL_LOAD_INTERV_P001_MT',
                              ...
                              end_date          => NULL,
                              enabled           => TRUE,
                              comments          => 'Carga de INTERV del día a partir de la réplica');
END;