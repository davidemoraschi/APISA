CREATE OR REPLACE PACKAGE BODY         "PKG_UPDATE_KPI_QUI" IS

--============================================================================--
--==========================MODULO DE QUIRURGICO==============================--
--============================================================================--

    PROCEDURE QUI_N_INTERV IS
    ...
    secuencia NUMBER;
    BEGIN
        SELECT SEQ_LANZAMIENTO.NEXTVAL INTO secuencia FROM dual;

          INSERT
          INTO CONTROL_LANZAMIENTO
            (
              ID,
              PROCEDIMIENTO,
              FECHA_INI
      ...
      DELETE FROM KPI_QUI_N_INTERV where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');
      --estado de intervencion 2 desprogramada, 4 suspendida
      INSERT INTO KPI_QUI_N_INTERV(FECHA,contador)
      select fecha,sum(contador)
        from
        (
            SELECT TRUNC (cab.fecha_hora_inicio, 'MM') FECHA, COUNT (1) AS contador
            FROM REP_HIS_OWN.ADM_QRF_IQ_PRE@SEE41DAE pre,
                REP_HIS_OWN.ADM_QRF_PQ_DET@SEE41DAE det,
                REP_HIS_OWN.ADM_QRF_PQ_CAB@SEE41DAE cab,
                REP_HIS_OWN.ADM_EPISODIO@SEE41DAE epi
            WHERE pre.episodio_id = epi.episodio_id
                ...
                AND (epi.fch_cierre is null OR
                (trunc(epi.fch_apertura,'dd') <> trunc(epi.fch_cierre,'dd')))
                AND TRUNC(cab.fecha_hora_inicio) >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM')
                AND pre.estado_id<>2 and pre.estado_id<>4
      ...
      UPDATE CONTROL_LANZAMIENTO SET fecha_fin=sysdate WHERE id=secuencia;
      COMMIT;
      exception when others then

          l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
          INSERT INTO KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_INTERV', l_mensaje_error);
            UPDATE CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_INTERV ERROR: '||l_mensaje_error) WHERE id=secuencia;
          COMMIT;

    END QUI_N_INTERV;

    PROCEDURE QUI_N_INTERV_FIRM IS
    ...
    BEGIN
      ...
      DELETE FROM KPI_QUI_N_INTERV_FIRM where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

      INSERT INTO KPI_QUI_N_INTERV_FIRM(FECHA,contador)
      select fecha,sum(contador)
        from
        (
        SELECT TRUNC (cab.fecha_hora_inicio, 'MM') FECHA, COUNT (1) AS contador
      ...
      COMMIT;
       ...
       exception when others then
          ...
          l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
          INSERT INTO KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_INTERV_FIRM', l_mensaje_error);
            UPDATE CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_INTERV_FIRM ERROR: '||l_mensaje_error) WHERE id=secuencia;
          COMMIT;

    END QUI_N_INTERV_FIRM;

    PROCEDURE QUI_N_INTERV_MENOS_UNO IS
    ...
    --Numero de intervenciones  programadas y realizadas en el ambito de hospitalizacion, con menos de 1 dia de estancia en el hospital.
        ...
        BEGIN
            SELECT SEQ_LANZAMIENTO.NEXTVAL INTO secuencia FROM dual;
              ...
              INTO CONTROL_LANZAMIENTO
                ...
                (
                  secuencia,
                  'QUI_N_INTERV_MENOS_UNO',
                  sysdate
                );
                COMMIT;

          DELETE FROM KPI_QUI_N_INTERV_MENOS_UNO where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

          INSERT INTO KPI_QUI_N_INTERV_MENOS_UNO(FECHA,contador)
          select fecha,sum(contador)
            from
            (
            SELECT TRUNC (cab.fecha_hora_inicio, 'MM') FECHA, COUNT (1) AS contador
        ...
        exception when others then
              ...
              l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
              INSERT INTO KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_INTERV_MENOS_UNO', l_mensaje_error);
                UPDATE CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_INTERV_MENOS_UNO ERROR: '||l_mensaje_error) WHERE id=secuencia;
              COMMIT;

        END QUI_N_INTERV_MENOS_UNO;

    PROCEDURE QUI_N_INTERV_FIRM_M_UNO IS
    ...
    BEGIN
        SELECT SEQ_LANZAMIENTO.NEXTVAL INTO secuencia FROM dual;
          ...
          INTO CONTROL_LANZAMIENTO
            ...
            (
              secuencia,
              'QUI_N_INTERV_FIRM_M_UNO',
              sysdate
            );
            COMMIT;

    DELETE FROM KPI_QUI_N_INTERV_FIRM_M_UNO where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

    INSERT INTO KPI_QUI_N_INTERV_FIRM_M_UNO(FECHA,contador)
      select fecha,sum(contador)
        from
        (
        SELECT TRUNC (cab.fecha_hora_inicio, 'MM') FECHA, COUNT (1) AS contador
    ...
    exception when others then
          ...
          l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
          INSERT INTO KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_INTERV_FIRM_M_UNO', l_mensaje_error);
            UPDATE CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_INTERV_FIRM_M_UNO ERROR: '||l_mensaje_error) WHERE id=secuencia;
          COMMIT;
    END QUI_N_INTERV_FIRM_M_UNO;

    PROCEDURE QUI_N_INTERV_CMA IS
    ...
    DELETE FROM KPI_QUI_N_INTERV_CMA where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

    INSERT INTO KPI_QUI_N_INTERV_CMA(FECHA,contador)
    select fecha,sum(contador)
    from
    (
        ...
        (
            (SELECT distinct cab.fecha_hora_inicio fecha, pre.iq_pre_id iqpre
                     FROM REP_HIS_OWN.ADM_QRF_IQ_PRE@SEE41DAE pre,
                              ...
                              REP_HIS_OWN.ADM_EPIS_DETALLE@SEE41DAE EPIDET,
                              REP_HIS_OWN.ADM_QRF_IQ_ICD_PRE@SEE41DAE ICD,
                              kpi_cma_2013 cma
                    where PRE.EPISODIO_ID = EPI.EPISODIO_ID
                    ...
                    and icd.iq_pre_id=pre.iq_pre_id
                      ...
                      AND epi.fch_cierre IS NOT NULL
                      AND (    epi.fch_apertura = epi.fch_cierre - 1
                               AND TO_CHAR (epi.hora_cierre, 'HH24') <= '15'
                               AND TO_CHAR (cab.fecha_hora_inicio, 'HH24') >= '15'
                              )
                      AND adm.destino_alta <> 7 AND adm.motivo_alta <> 6
                      AND TRUNC (cab.fecha_hora_inicio) >=
    ...
    exception when others then
            ...
            l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
            INSERT INTO KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_INTERV_CMA', l_mensaje_error);
            UPDATE CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_INTERV_CMA ERROR: '||l_mensaje_error) WHERE id=secuencia;
            COMMIT;

    END QUI_N_INTERV_CMA;

    PROCEDURE QUI_N_INTERV_FIRM_CMA IS
    ...
    BEGIN
        SELECT SEQ_LANZAMIENTO.NEXTVAL INTO secuencia FROM dual;
          ...
          INTO CONTROL_LANZAMIENTO
            ...
            (
              secuencia,
              'QUI_N_INTERV_FIRM_CMA',
              sysdate
            );
            COMMIT;

    DELETE FROM KPI_QUI_N_INTERV_FIRM_CMA where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');
    ...
    UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate WHERE id=secuencia;
    COMMIT;

    exception when others then

        l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
        INSERT INTO OWN_KPI.KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_INTERV_FIRM_CMA', l_mensaje_error);
        UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_INTERV_FIRM_CMA ERROR: '||l_mensaje_error) WHERE id=secuencia;
        COMMIT;
    END QUI_N_INTERV_FIRM_CMA;

    PROCEDURE QUI_N_INTERV_AMBU IS
    ...
    BEGIN
        SELECT own_kpi.SEQ_LANZAMIENTO.NEXTVAL INTO secuencia FROM dual;
          ...
          INSERT
          INTO own_kpi.CONTROL_LANZAMIENTO
            (
              ID,
              PROCEDIMIENTO,
              FECHA_INI
    ...
    DELETE FROM OWN_KPI.KPI_QUI_N_INTERV_AMBU where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');
    ...
    (
        (
            ...
            (
                ...
                FROM
                (
                    SELECT cab.fecha_hora_inicio fecha
                         FROM REP_HIS_OWN.ADM_QRF_IQ_PRE pre,
                              REP_HIS_OWN.ADM_QRF_PQ_DET det,
                              REP_HIS_OWN.ADM_QRF_PQ_CAB cab,
                              REP_HIS_OWN.ADM_EPISODIO epi,
        ...
        (
            ...
            (
                ...
                FROM
                (
                    SELECT cab.fecha_hora_inicio fecha, pre.iq_pre_id iqpre
                    FROM REP_HIS_OWN.ADM_QRF_IQ_PRE pre,
                        REP_HIS_OWN.ADM_QRF_PQ_DET det,
                        REP_HIS_OWN.ADM_QRF_PQ_CAB cab,
                        REP_HIS_OWN.ADM_EPISODIO epi,
    ...
    exception when others then
            ...
            l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
            INSERT INTO OWN_KPI.KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_INTERV_AMBU', l_mensaje_error);
            UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_INTERV_AMBU ERROR: '||l_mensaje_error) WHERE id=secuencia;
            COMMIT;
    END QUI_N_INTERV_AMBU;

    PROCEDURE QUI_N_INTERV_FIRM_AMBU IS
    ...
    BEGIN
        SELECT own_kpi.SEQ_LANZAMIENTO.NEXTVAL INTO secuencia FROM dual;
          ...
          INTO own_kpi.CONTROL_LANZAMIENTO
            ...
            (
              secuencia,
              'QUI_N_INTERV_FIRM_AMBU',
              sysdate
            );
            COMMIT;

    DELETE FROM OWN_KPI.KPI_QUI_N_INTERV_FIRM_AMBU where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');
    ...
    exception when others then
            ...
            l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
            INSERT INTO OWN_KPI.KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_INTERV_FIRM_AMBU', l_mensaje_error);
            UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_INTERV_FIRM_AMBU ERROR: '||l_mensaje_error) WHERE id=secuencia;
            COMMIT;
    END QUI_N_INTERV_FIRM_AMBU;

    PROCEDURE QUI_N_INTERV_URG_HOSP IS
    ...
    BEGIN
        SELECT own_kpi.SEQ_LANZAMIENTO.NEXTVAL INTO secuencia FROM dual;
          ...
          INTO own_kpi.CONTROL_LANZAMIENTO
            ...
            (
              secuencia,
              'QUI_N_INTERV_URG_HOSP',
              sysdate
            );
            COMMIT;

        DELETE FROM OWN_KPI.KPI_QUI_N_INTERV_URG_HOSP where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');
        ...
        FROM rep_sidca_own.hce_episodio_clinico ep
         inner join rep_his_own.adm_qrf_iq_post POST on to_char(POST.episodio_id) = ep.nepisodio
         inner join rep_his_own.adm_episodio epi_eg on epi_eg.episodio_id = POST.episodio_id
       WHERE ep.nepisodio IS NOT NULL
         AND ep.tipo_episodio = 'H'
         AND epi_eg.modalidad_asist = 1 --hospitalizacion
         AND ep.fch_alta > ep.fch_ingreso
    ...
    exception when others then
            ...
            l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
            INSERT INTO OWN_KPI.KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_INTERV_URG_HOSP', l_mensaje_error);
            UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_INTERV_URG_HOSP ERROR: '||l_mensaje_error) WHERE id=secuencia;

            COMMIT;

    END QUI_N_INTERV_URG_HOSP;

    PROCEDURE QUI_N_INTERV_URG_HDQ IS
    ...
    BEGIN
        SELECT own_kpi.SEQ_LANZAMIENTO.NEXTVAL INTO secuencia FROM dual;
          ...
          INTO own_kpi.CONTROL_LANZAMIENTO
            ...
            (
              secuencia,
              'QUI_N_INTERV_URG_HDQ',
              sysdate
            );
            COMMIT;

    DELETE FROM OWN_KPI.KPI_QUI_N_INTERV_URG_HDQ where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

    INSERT INTO OWN_KPI.KPI_QUI_N_INTERV_URG_HDQ(FECHA,contador)
    SELECT TRUNC (fecha, 'MM') AS fecha, COUNT (1) AS contador
    FROM
        (
         SELECT /*+ index (ep HCE_EPIS_CLI_NEPISODIO) */post.FH_INICIO AS fecha
            FROM REP_SIDCA_OWN.HCE_EPISODIO_CLINICO ep,
            ...
            AND ep.destino_alta <> 7 AND ep.tipo_alta <> 'T'
            AND ep.fch_alta IS NOT NULL
            AND to_char(post.episodio_id) = ep.nepisodio
            AND post.iq_pre_id IS NULL --que no haya parte asociado
            AND (trunc(ep.fch_ingreso) = trunc(ep.fch_alta))
            AND
             (ep.cod_servicio_alta is not null and ep.cod_servicio_alta=uf.uf_codigo
                OR
              ep.cod_servicio_alta is null and ep.cod_servicio_ingreso=uf.uf_codigo)
            AND TRUNC (post.FH_INICIO, 'MM') >= TRUNC (ADD_MONTHS (SYSDATE, -12),'MM')
            UNION ALL
            ...
            AND icd.principal=1
            AND (
                 trunc(ep.fch_ingreso,'dd') = trunc(ep.fch_alta,'dd') - 1
                 AND TO_CHAR (ep.fch_ingreso, 'HH24') <= '15'
                 AND TO_CHAR (ep.fch_alta, 'HH24') >= '15'
                )
            AND
             (ep.cod_servicio_alta is not null and ep.cod_servicio_alta=uf.uf_codigo
                OR
    ...
    exception when others then
            ...
            l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
            INSERT INTO OWN_KPI.KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_INTERV_URG_HDQ', l_mensaje_error);
            UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_INTERV_URG_HDQ ERROR: '||l_mensaje_error) WHERE id=secuencia;
            COMMIT;
    END QUI_N_INTERV_URG_HDQ;

    PROCEDURE QUI_N_INTERV_URG_URG IS
    ...
    BEGIN
        SELECT own_kpi.SEQ_LANZAMIENTO.NEXTVAL INTO secuencia FROM dual;
          ...
          INTO own_kpi.CONTROL_LANZAMIENTO
            ...
            (
              secuencia,
              'QUI_N_INTERV_URG_URG',
              sysdate
            );
            COMMIT;

    DELETE FROM OWN_KPI.KPI_QUI_N_INTERV_URG_URG where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');
    ...
    select fecha, COUNT (1) AS contador
        ...
        SELECT TRUNC (post.FH_INICIO, 'MM') AS fecha
                FROM rep_sidca_own.hce_episodio_clinico ep
                      ...
                      inner join rep_his_own.adm_episodio epi_eg on epi_eg.episodio_id = POST.episodio_id
                WHERE
                     ep.nepisodio IS NOT NULL
                 AND tipo_episodio = 'U'
                 AND epi_eg.modalidad_asist = 3 --urgencias
                 AND ep.fch_alta > ep.fch_ingreso
                 AND post.iq_pre_id IS NULL --que no haya parte asociado
                 AND TRUNC (post.FH_INICIO, 'MM') >= TRUNC (ADD_MONTHS (SYSDATE, -12),'MM')
                union all
    ...
    exception when others then
            ...
            l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
            INSERT INTO OWN_KPI.KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_INTERV_URG_URG', l_mensaje_error);
            UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_INTERV_URG_URG ERROR: '||l_mensaje_error) WHERE id=secuencia;
            COMMIT;

    END QUI_N_INTERV_URG_URG;

    PROCEDURE QUI_N_SUSP_HOSP IS
    ...
    DELETE FROM OWN_KPI.KPI_QUI_N_SUSP_HOSP where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

    INSERT INTO OWN_KPI.KPI_QUI_N_SUSP_HOSP(FECHA,contador,nombre,codigo)
    select fecha,sum(contador),uf_nombre,uf_codigo
    from
    (
        SELECT   TRUNC (desp.fh_desprog, 'MONTH') fecha, COUNT (1) AS contador,
                 uf.uf_padre_nombre as uf_nombre, uf.uf_padre as uf_codigo
    ...
    DELETE FROM KPI_QUI_N_SUSP_HOSP_MOT where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

    INSERT INTO KPI_QUI_N_SUSP_HOSP_MOT(FECHA,contador,motivo,motivo_id)
    select fecha,sum(contador),motivo,motivo_id
    from
    (
    ...
    DELETE FROM KPI_QUI_N_SUSP_HOSP_MOT_UF where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

    INSERT INTO OWN_KPI.KPI_QUI_N_SUSP_HOSP_MOT_UF(FECHA,uf_nombre,uf_codigo,motivo,motivo_id,contador)
    select fecha,uf_nombre,uf_codigo,motivo,motivo_id,sum(contador) as contador
    from
    (
        SELECT   TRUNC (desp.fh_desprog, 'MONTH') fecha, COUNT (1) AS contador,
                 uf.uf_padre_nombre as uf_nombre, uf.uf_padre as uf_codigo,
           ...
           WHERE mot.mot_desprog_id = desp.mot_desprog_id
             ...
             AND cab.uni_funcional_id = uf.uf_codigo
             AND TRUNC (desp.fh_desprog) >= TRUNC (ADD_MONTHS (SYSDATE, -12), 'MM')
        GROUP BY TRUNC (desp.fh_desprog, 'MONTH'), uf.uf_padre_nombre, uf.uf_padre, MOT.DESC_MOT_DESPROG, MOT.MOT_DESPROG_ID
        union all
        --aquellas intervenciones sin episodio asociado
        SELECT   TRUNC (desp.fh_desprog, 'MONTH') fecha, COUNT (1) AS contador,
                 uf.uf_padre_nombre as uf_nombre, uf.uf_padre as uf_codigo,
    ...
    GROUP BY fecha,uf_nombre,uf_codigo,motivo,motivo_id;
    ...
    exception when others then
            ...
            l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
            INSERT INTO OWN_KPI.KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_SUSP_HOSP', l_mensaje_error);
            UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_SUSP_HOSP ERROR: '||l_mensaje_error) WHERE id=secuencia;
            COMMIT;

    END QUI_N_SUSP_HOSP;

    PROCEDURE QUI_N_SUSP_AMBU IS
    ...
    DELETE FROM OWN_KPI.KPI_QUI_N_SUSP_AMBU where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

    INSERT INTO OWN_KPI.KPI_QUI_N_SUSP_AMBU(FECHA,contador,nombre,codigo)
    select fecha,sum(contador),uf_nombre,uf_codigo
    from
    (
        SELECT   TRUNC (desp.fh_desprog, 'MONTH') fecha, COUNT (1) AS contador,
    ...
    DELETE FROM KPI_QUI_N_SUSP_AMBU_MOT where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

    INSERT INTO KPI_QUI_N_SUSP_AMBU_MOT(FECHA,contador,motivo,motivo_id)
    select fecha,sum(contador),motivo,motivo_id
    from
    (
        SELECT TRUNC (DESP.FH_DESPROG, 'MONTH') FECHA,
    ...
    DELETE FROM OWN_KPI.KPI_QUI_N_SUSP_AMBU_MOT_UF where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

    INSERT INTO OWN_KPI.KPI_QUI_N_SUSP_AMBU_MOT_UF(FECHA,uf_nombre,uf_codigo,motivo,motivo_id,contador)
    select fecha,uf_nombre,uf_codigo,motivo,motivo_id,sum(contador)
    from
    (
        SELECT   TRUNC (desp.fh_desprog, 'MONTH') fecha, COUNT (1) AS contador,
                 uf.uf_padre_nombre as uf_nombre, uf.uf_padre as uf_codigo,
    ...
    exception when others then
            ...
            l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
            INSERT INTO OWN_KPI.KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_SUSP_AMBU', l_mensaje_error);
            UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_SUSP_AMBU ERROR: '||l_mensaje_error) WHERE id=secuencia;
            COMMIT;

    END QUI_N_SUSP_AMBU;

    PROCEDURE QUI_N_SUSP_MOT_P_UF IS
    ...
    DELETE FROM own_kpi.KPI_QUI_N_SUSP_MOT_UF where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');
    ...
    exception when others then
            ...
            l_mensaje_error := SUBSTR (DBMS_UTILITY.format_error_stack (), 1, 1999);
            INSERT INTO OWN_KPI.KPI_ERRORES (fecha_error, KPI_NAME, error) VALUES (SYSDATE, 'QUI_N_SUSP_MOT_P_UF', l_mensaje_error);
            UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_N_SUSP_MOT_P_UF ERROR: '||l_mensaje_error) WHERE id=secuencia;
            COMMIT;

    END QUI_N_SUSP_MOT_P_UF;

    PROCEDURE QUI_REND_QRF_H_DISP_AH IS
          ...
          INSERT INTO OWN_KPI.KPI_QUI_REND_QRF_H_DISP_AH (fecha, contador)
                 SELECT TRUNC (fecha_hora_fin, 'mm'), SUM (contador) AS contador
                    FROM (SELECT DISTINCT
                                 pq_cab_id,
                                 (fecha_hora_fin - fecha_hora_inicio) * 24 AS contador,
                                 fecha_hora_fin
                            FROM REP_HIS_OWN.ADM_QRF_PQ_CAB cab
                                 INNER JOIN REP_HIS_OWN.ADM_QRF_PQ_DET det USING (pq_cab_id)
                                 INNER JOIN REP_HIS_OWN.ADM_QRF_LOCAL qrf ON cab.quirofano = qrf.ID
                                 ...
                                 AND (SELECT COUNT (1)
                                        FROM REP_HIS_OWN.ADM_QRF_IQ_PRE pre
                                             INNER JOIN REP_HIS_OWN.ADM_QRF_PQ_DET det
                                                USING (iq_pre_id)
                                       WHERE det.pq_cab_id = pq_cab_id AND det.orden <> 65000) >
                                        0
                                 AND det.orden <> 65000)
    ...
    EXCEPTION
       ...
       THEN
          ...
          UPDATE own_kpi.CONTROL_LANZAMIENTO
             SET fecha_fin = SYSDATE,
                 PROCEDIMIENTO =
                    ('QUI_REND_QRF_H_DISP_AH ERROR: ' || l_mensaje_error)
           WHERE id = secuencia;
    ...
    BEGIN
       ...
       COMMIT;
          ...
          INSERT INTO OWN_KPI.KPI_QUI_REND_QRF_H_UTIL_AH (fecha, contador)
               SELECT TRUNC (fecha_hora_fin, 'mm') AS fecha,
                      SUM ( (fh_fin_ocup - fh_inicio_ocup) * 24) AS contador
                 FROM REP_HIS_OWN.ADM_QRF_IQ_POST POST
                      INNER JOIN REP_HIS_OWN.ADM_QRF_PQ_DET det USING (iq_post_id)
                      INNER JOIN REP_HIS_OWN.ADM_QRF_PQ_CAB cab USING (pq_cab_id)
                      INNER JOIN REP_HIS_OWN.ADM_QRF_LOCAL qrf ON cab.quirofano = qrf.ID
                WHERE  fecha_hora_fin >= TRUNC (ADD_MONTHS (SYSDATE, -12), 'mm')
                   AND fecha_hora_fin <= SYSDATE - 1
                   AND qrf.consulta = 0
    ...
    EXCEPTION
       ...
       THEN
          ...
          UPDATE own_kpi.CONTROL_LANZAMIENTO
             SET fecha_fin = SYSDATE,
                 PROCEDIMIENTO =
                    ('QUI_REND_QRF_H_UTIL_AH ERROR: ' || l_mensaje_error)
           WHERE id = secuencia;
    ...
    BEGIN
       ...
       INSERT INTO own_kpi.KPI_qui_rend_qrf_ind_util_AH
                   (fecha, contador)
          SELECT TRUNC (fecha, 'mm'),
                 NVL (util.contador / disp.contador * 100, 0) AS contador
            FROM OWN_KPI.KPI_qui_rend_qrf_h_disp_AH disp LEFT JOIN OWN_KPI.KPI_qui_rend_qrf_h_util_AH util
                 USING (fecha)
                 WHERE fecha >= TRUNC (add_months(sysdate,-12), 'mm');
    ...
    EXCEPTION
       ...
       THEN
          ...
          INSERT INTO own_kpi.kpi_errores
                      (fecha_error, kpi_name, error
                      )
               VALUES (SYSDATE, 'QUI_REND_QRF_IND_UTIL_AH', l_mensaje_error
                      );
            UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_REND_QRF_IND_UTIL_AH ERROR: '||l_mensaje_error) WHERE id=secuencia;
          COMMIT;
    END QUI_REND_QRF_IND_UTIL_AH;


    PROCEDURE QUI_REN_PORCENT_ELIM_AH IS
       ...
       IS
            SELECT COUNT (*) as contador
              FROM REP_HIS_OWN.ADM_QRF_IQ_PRE PRE
                   INNER JOIN REP_HIS_OWN.ADM_QRF_PQ_DET det USING (iq_pre_id)
                   INNER JOIN REP_HIS_OWN.ADM_QRF_PQ_CAB cab USING (pq_cab_id)
                   INNER JOIN REP_HIS_OWN.ADM_QRF_LOCAL qrf ON cab.quirofano = qrf.ID
       ...
       IS
            SELECT COUNT (*) AS contador
              FROM REP_HIS_OWN.ADM_QRF_IQ_POST POST
                   ...
                   INNER JOIN REP_HIS_OWN.ADM_QRF_PQ_CAB cab USING (pq_cab_id)
                   INNER JOIN REP_HIS_OWN.ADM_QRF_LOCAL qrf ON cab.quirofano = qrf.ID
             WHERE     (   (    POST.fh_fin >= TRUNC (fecha, 'mm')
                            AND POST.fh_fin <= LAST_DAY (fecha)
                            AND qrf.consulta = 0
                            AND (   (POST.fh_fin_ocup - POST.fh_inicio_ocup) * 24 * 60 >= 720
                                 OR (POST.fh_fin_ocup - POST.fh_inicio_ocup) <= 0))
    ...
    BEGIN
       ...
       LOOP
          ...
          FOR fila
             IN (  SELECT TRUNC (cab.FECHA_HORA_FIN, 'mm') AS fecha,
                          COUNT (*) AS contador
                     FROM REP_HIS_OWN.ADM_QRF_IQ_PRE PRE
                          INNER JOIN REP_HIS_OWN.ADM_QRF_PQ_DET det USING (iq_pre_id)
                          INNER JOIN REP_HIS_OWN.ADM_QRF_PQ_CAB cab USING (pq_cab_id)
          ...
          LOOP
            ...
            THEN
             INSERT INTO OWN_KPI.KPI_qui_ren_porcent_eli_AH (fecha,
                                                            contador)
                     VALUES (
                               TRUNC (
                                  TO_DATE (clave, 'dd/mm/yyyy'),
                                  'mm'),
                               datos (clave));
            END IF;
    ...
    EXCEPTION
       ...
       THEN
          ...
          UPDATE own_kpi.CONTROL_LANZAMIENTO
             SET fecha_fin = SYSDATE,
                 PROCEDIMIENTO =
                    ('QUI_REN_PORCENT_ELIM_AH ERROR: ' || l_mensaje_error)
           WHERE id = secuencia;

          COMMIT;
    END QUI_REN_PORCENT_ELIM_AH;

    PROCEDURE QUI_EST_MED_PREOP_PROG IS
    ...
    BEGIN
       ...
       LOOP
          ...
          IF (    objeto.fintervencion >= objeto.fingreso
              AND objeto.fintervencion < objeto.fingreso + 30)
          THEN
       ...
       LOOP
          ...
          THEN
             tablaMeses (clavetrunc) :=
                  tablaMeses (clavetrunc)
                + (oFecha.fintervencion - oFecha.fingreso);
    ...
    EXCEPTION
       ...
       THEN
          ...
          UPDATE own_kpi.CONTROL_LANZAMIENTO
             SET fecha_fin = SYSDATE,
                 PROCEDIMIENTO =
                    ('QUI_EST_MED_PREOP_PROG ERROR: ' || l_mensaje_error)
           WHERE id = secuencia;

          COMMIT;
    END QUI_EST_MED_PREOP_PROG;

    PROCEDURE QUI_EST_MED_PREOP_URG_DIF IS
    ...
    BEGIN
        SELECT own_kpi.SEQ_LANZAMIENTO.NEXTVAL INTO secuencia FROM dual;
          ...
          INTO own_kpi.CONTROL_LANZAMIENTO
            ...
            (
              secuencia,
              'QUI_EST_MED_PREOP_URG_DIF',
              sysdate
            );
            COMMIT;

       DELETE
       FROM own_kpi.KPI_QUI_EST_MED_PREOP_URG_DIF
       where fecha >= TRUNC(ADD_MONTHS(SYSDATE, -12),'MM');

       INSERT INTO own_kpi.KPI_QUI_EST_MED_PREOP_URG_DIF(fecha,contador)
        SELECT datos.*
          FROM (  SELECT TRUNC (fintervencion, 'MM') AS fecha,
                         (SUM (fintervencion - fingreso) / COUNT (1)) AS contador
                    FROM (SELECT *
                            FROM (                         --PARTE DE EPISODIO INTERNO
                                  SELECT fingreso,
                                         fintervencion,
                                         fintervencion - fingreso
                                    FROM (SELECT nvl2(adm.FCH_ADMISION_URGENCIAS,to_date(to_char(adm.fch_admision_urgencias,'dd/mm/yyyy'),'dd/mm/yyyy'), ep.fch_apertura) AS fingreso,
                                                 TRUNC (cab.fecha_hora_inicio)
                                                    AS fintervencion
                                            FROM REP_HIS_OWN.ADM_EPISODIO ep,
                                           ...
                                           WHERE pre.episodio_id = ep.episodio_id
                                                 ...
                                                 AND det.pq_cab_id = cab.pq_cab_id
                                                 AND cab.uni_funcional_id =
                                                        uf.uf_codigo
                                                 AND pre.estado_id <> 2
                                                 AND pre.estado_id <> 4
                                                 AND nvl2(adm.FCH_ADMISION_URGENCIAS,to_date(to_char(adm.fch_admision_urgencias,'dd/mm/yyyy'),'dd/mm/yyyy'), ep.fch_apertura) >=
                                                        ADD_MONTHS (
                                                           TRUNC (SYSDATE, 'MM'),
                                                           -12)
                                                 AND CAB.UNI_FUNCIONAL_ID =
                                                        uf.uf_codigo)
                                   WHERE     fintervencion >= fingreso
                                         AND fintervencion < fingreso + 30)
                          UNION ALL
                                    ...
                                    WHERE     det.iq_pre_id = pre.iq_pre_id
                                          ...
                                          AND nvl2(adm.FCH_ADMISION_URGENCIAS,to_date(to_char(adm.fch_admision_urgencias,'dd/mm/yyyy'),'dd/mm/yyyy'), ep.fch_apertura) >=
                                                 ADD_MONTHS (TRUNC (SYSDATE, 'MM'),
                                                             -12)
                                          AND CAB.UNI_FUNCIONAL_ID = uf.uf_codigo
                                          AND own_kpi.episodio_sin_asociar (
                                                 pre.id_usuario,
                                                 cab.fecha_hora_inicio) =
                                                 ep.episodio_id
                                          AND (   own_kpi.modalidad_episodio (
                                                     pre.id_usuario,
                                                     cab.fecha_hora_inicio) = 1
                                               OR own_kpi.modalidad_episodio (
                                                     pre.id_usuario,
                                                     cab.fecha_hora_inicio) = 2)
                                          AND ep.episodio_id = epdet.episodio_id
                                          AND epdet.referencia_id = adm.admision_id
    ...
    EXCEPTION
       ...
       THEN
        ...
        INTO own_kpi.kpi_errores
          ...
          (
            SYSDATE,
            'KPI_QUI_EST_MED_PREOP_URG_DIF',
            l_mensaje_error
          );
          UPDATE own_kpi.CONTROL_LANZAMIENTO SET fecha_fin=sysdate,PROCEDIMIENTO=('QUI_EST_MED_PREOP_URG_DIF ERROR: '||l_mensaje_error) WHERE id=secuencia;
        COMMIT;
    END QUI_EST_MED_PREOP_URG_DIF;

    PROCEDURE QUI_EST_MED_PREOP_GLO IS
       --Tiempo medio desde un ingreso en hospitalizacion hasta la intervencion quirurgica programada con indicacion "preingreso".
               ...
               FROM (SELECT *
                       FROM (                         --PARTE DE EPISODIO INTERNO
                             SELECT 0-episodio_id as episodio_id,fingreso, fintervencion
                               FROM (SELECT ep.episodio_id,nvl2(adm.FCH_ADMISION_URGENCIAS,to_date(to_char(adm.fch_admision_urgencias,'dd/mm/yyyy'),'dd/mm/yyyy'), ep.fch_apertura) AS fingreso,
                                            TRUNC (cab.fecha_hora_inicio)
                                               AS fintervencion
                                       FROM REP_HIS_OWN.ADM_EPISODIO ep,
                                            REP_HIS_OWN.ADM_EPIS_DETALLE epdet,
    ...
    EXCEPTION
       ...
       THEN
          ...
          UPDATE own_kpi.CONTROL_LANZAMIENTO
             SET fecha_fin = SYSDATE,
                 PROCEDIMIENTO =
                    ('QUI_EST_MED_PREOP_GLO ERROR: ' || l_mensaje_error)
           WHERE id = secuencia;

          COMMIT;
    END QUI_EST_MED_PREOP_GLO;

    PROCEDURE LANZAR_TODO IS
