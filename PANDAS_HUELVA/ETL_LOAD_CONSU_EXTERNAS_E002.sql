/* Formatted on 28/05/2013 9:52:39 (QP5 v5.163.1008.3004) */
...
BEGIN
   DBMS_SCHEDULER.DROP_JOB (job_name => 'ETL_LOAD_CONSU_EXTERNAS_E002');
END;

ALTER SESSION SET TIME_ZONE = 'Europe/Madrid';

BEGIN
   DBMS_SCHEDULER.CREATE_JOB (job_name          => 'ETL_LOAD_CONSU_EXTERNAS_E002',
                              job_type          => 'EXECUTABLE',
                              job_action        => '/u01/app/oracle/consultas_externas/descarga_citaweb_consultas.sh',
                              ...
                              end_date          => NULL,
                              enabled           => FALSE,
                              comments          => 'Carga de datos de consultas desde CITAWEB');
