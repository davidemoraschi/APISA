/* Formatted on 03/05/2013 12:14:36 (QP5 v5.163.1008.3004) */
...
SELECT                                                                                                                 --NUM_CAMA,
  ...
  FROM REP_HIS_OWN.COM_UBICACION_GESTION_LOCAL@EXP
       JOIN REP_PRO_EST.UBICACIONES@VAL
          ON (CODIGO_ESTRUCTURA = UBI_CODIGO)
       JOIN REP_PRO_EST.TIPOS_UBICACION@VAL
       ...
       LEFT JOIN (SELECT UBICACION_ORIGEN NUM_CAMA FROM REP_HIS_OWN.COM_UBICACION_AISLADA@EXP
                  UNION
                  SELECT UBICACION_AFECTADA NUM_CAMA FROM REP_HIS_OWN.COM_UBICACION_AISLADA@EXP)
          ON (ID = NUM_CAMA)
